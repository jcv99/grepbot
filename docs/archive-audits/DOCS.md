...
per-feature plans `01`–`25` predate v1.6; most of them shipped (see §7 below).
...
- Stats → Preflight → **academy read path** row: no `UNREADABLE` field. `real queue UNREADABLE` there means only the open town can be researched.
...
(core.js). caller that skips the same check still won't post while
...
renamed/missing getter must yield `blind`, never a fabricated `0` /
...
support send / wonder donation / dodge that times out may have
...
3:15:24 PM instant: completed 0/1.. (repeats ~every 1–3 s for the whole skip window)
```
...
(src/journal.js:96 `jrnPush`). key comes from `jrnTag` (src/journal.js:50):
...
most likely triggers for `completeInstant` posts on a live world right now:
...
### 2. bot never backs off when posts are short-circuited
...
order is genuinely still free in the model because the server never
completed it — the bot's posts were rejected, so the building clock
...
```
10 s `IB_CHECK_MS` interval (src/boot.js:113) is the recovery path; the 3 s
accelerator only earns its keep when something moved.

### B. Skip remembered orders in `ibScan`, before the batch is announced

This is the change that stops the loop. A/B/C only quiet it: the scan
...
game's own user-guide selector is `.tech_tree_box .button_upgrade[data-research_id=X]`.
...
blind-verdict refactor is still worth doing for the CLAUDE.md invariant —
not as the fix.
...
MM fallback provably carries only the current town: `research_orders` is a
...
attribute-based and will match. tech tree template itself is
...
sits in flow and above `.tech_tree_box` overlays. real gap is only that
the build lane has explicit anchor logic (`src/native-ui.js:669`) and the
research lane does a bare `tile.appendChild(ctl)`. tech-tree cell's own
...
open window `window_c970` (line 2755) as **sibling children of `<body>`**. `subtree:true` observer on `#ui_box` cannot see an insertion in a sibling
...
herring — `childList` mutations are not gated by `attributeFilter`. fix is
...
third verifier established the full trigger inventory: the only trigger that
...
produces `researchN === 0`, and `nativeUiScan` (`src/native-ui.js:771`)
...
Academy tech tree markup is **server-rendered with the window payload** and
...
sweep, then the working copy (open town only). empty result only counts as
...
class hooks the game itself binds (`.btn_upgrade`, `.button_upgrade`,
 `.research_icon`). class-only node still has to resolve to a real GameData
...
the bot's own repaints do not feed it. 5s `scheduleNativeUiScan` is now
