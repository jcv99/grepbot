...
data/                # smoke driver output — ephemeral, not in archive/
grepbot.user.js.prod # pinned-prod build artifact (kept beside current)
```

Repo root is deliberately minimal: `grepbot.user.js`, `build.py`, `src/`,
`CLAUDE.md`. `data/` is regenerated by the smoke driver (delete when done);
`grepbot.user.js.prod` is a stable reference build, not the working copy.
...
**Pre-edit smoke** (optional but cheap): `node .claude/skills/run-grepbot/driver.mjs`
builds, `node --check`s, boots the artifact in headless Chromium over CDP,
clicks every panel tab, runs Preflight, and screenshots each step. Init smoke
only — does not exercise bridge posts. REPL: `… driver.mjs repl` for live
panel poking. Full detail: `.claude/skills/run-grepbot/SKILL.md`.
...
1. Bump `@version` in `src/header.js` when behavior changes. Gate #3 in
   `build.py` warns (not fatal) when `src/` changes but the header version
   does not — bump it explicitly so the warning stays meaningful, don't
   ignore it.
2. `python3 build.py` — gates 1 (dup decls), 2 (`node --check`), 4 (ASCII
   escape) are fatal. Exit 1 means the artifact was NOT written.
