...
[driver] tabs: overview:shown attack:shown reinforce:shown train:shown spy:shown intel:shown config:shown stats:shown log:shown
[driver] test exports: 265
[driver] preflight: 66 lines -> /…/data/smoke/1786747983519-preflight.txt
[driver] screenshot: /…/data/smoke/1786747983519-preflight.png
[driver] preflight rows: 55 total, 0 FAIL, 10 warn
...
6. clicks all 9 tabs (`overview` `attack` `reinforce` `train` `spy`
   `intel` `config` `stats` `log`), screenshots each, asserts each
   section unhides
7. asserts `window.__grepbotTest` published >50 internals, then seeds
   `state.farmOptionMap` / `state.ibResearch` through it
8. opens `Actions > Preflight`, dumps the Stats output to `.txt`, then
   re-runs `preflightRun()` over the internals handle and asserts
   **0 FAIL rows** (WARN rows allowed)
...
Tab ids: `overview attack reinforce train spy intel config stats log`
(nine, in nav order; matches `TABS` at `driver.mjs:48` and `TAB_GROUPS` at
`src/ui.js:354`). Queue Center lanes are `QTABS` at `driver.mjs:50`.
...
- **Preflight must report 0 FAIL rows.** The stub fakes two towns, one
  lootable village, csrf, `GameData` units/buildings/researches, favor and
  an attack spot, and the driver seeds `state.farmOptionMap` +
  `state.ibResearch` before clicking Preflight (a returning user would have
  them in storage; the stub cannot learn from traffic). WARN rows are fine —
  they mark unlearned templates. A FAIL row means a read path broke or the
  stub drifted from it; fix the stub or the code, not the assertion. Under
  `--notest` there is no handle to seed with, so the 0-FAIL assert is
  skipped. Preflight sends nothing — safe to run anywhere.
